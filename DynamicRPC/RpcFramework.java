...
public class RpcFramework {
	...
	public static <T> T refer(final Class<T> interfaceClass, final String host, final int port) throws Exception {
		...
		ServiceHandler serviceHandler = new ServiceHandler(host, port);

		//利用动态代理，对每个接口类的方法调用进行的隐藏
		return (T) Proxy.newProxyInstance(interfaceClass.getClassLoader(), new Class<?>[]{interfaceClass}, serviceHandler);
	}
	...
	private static class ServiceHandler implements InvocationHandler {
		...
		ServiceHandler(String host, int port) {
			this.host = host;
			this.port = port;
		}
